...
function add_bucketed_bars(row, pr) {
	...
	bucketed_statuses.set("continuous-integration/travis",
		{bar_width: 10, avatar: '', items: []});
	bucketed_statuses.set("ci/",
		{bar_width: 10, avatar: '', items: []});
	bucketed_statuses.set("pr/",
		{bar_width: 10, avatar: '', items: []});
	bucketed_statuses.set("unknown",
		{bar_width: 10, avatar: '', items: []});
	...
	for (let [bucket_name, bucket] of bucketed_statuses) {
		bucket.bar_width = Math.round(bucket.items.length / total_items * total_width);
	...
	for (let [bucket_name, bucket] of bucketed_statuses) {
		bucket.progress = {
			good: bucket.items.reduce((pre, curr) => (curr.state === "SUCCESS") ? ++pre : pre, 0),
			pending: bucket.items.reduce((pre, curr) => (curr.state === "PENDING") ? ++pre : pre, 0),
...
function bucket_bar(statuses) {
	...
	for (let [bucket_name, bucket] of statuses) {
		...
		// red background
		let total = make_node('rect', {x: used_width, y: 0, width: bucket.bar_width, height: height});
		total.style.fill = red;
		svg.appendChild(total);

		// yellow pending
		let pending_width = (bucket.progress.good + bucket.progress.pending) * scaler;
		let pending = make_node('rect', {x: used_width, y: 0, width: pending_width, height: height});
		pending.style.fill = yellow;
		svg.appendChild(pending);

		// green goods
		let progress_width = bucket.progress.good * scaler;
		let progress_rect = make_node('rect', {x: used_width, y: 0, width: progress_width, height: height});
		progress_rect.style.fill = green;
		svg.appendChild(progress_rect);

		// Spacer bar
		if (index < statuses.size - 1) {
			let spacer_rect = make_node('rect', {x: used_width + bucket.bar_width, y: 0, width: spacer, height: height});
...
function build_status_main() {
	...
	if (!GITHUB_OAUTH) {
		...
		console.error("Couldn't fetch up to date progress bars (no GitHub OAuth token, add one in settings)");
		return;
	}
	...
	if (window.location.href.includes('github.com/pulls')) {
		...
		for (let i = 0; i < rows.length; i++) {
			let row = rows[i];
			let combo = row.querySelector('.v-align-middle.muted-link.h4.pr-1').innerText.split('/');
			prs.push({
				'num': parse_num(row),
				'user': combo[0],
				'repo': combo[1],
		...
		status_request(url, {
			...
			success: (data) => {
				...
				for (let repo in repos) {
					...
					for (let pr in prs) {
						prs_and_id.push({
							"id": pr_id(repo.split('_')[0], repo.split('_')[1], pr.replace('p', '')),
							"pr": prs[pr]
						})
					}
				}
